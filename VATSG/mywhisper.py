...
def transcribe_fast_whisper_differently(file, option, uiwrapper):
    ...
    if get_byte_size(src_lang_sentence) > 40960:
        ...
        trg_lang = ''.join(trg_lang_sentences)
...
def transcribe_stable_whisper(file, option, uiwrapper):
    def makesrt(timestamp, strs):
        count = 1
        results = strs.splitlines()
        for time, result in zip(timestamp, results):
    ...
    def progress_notify(seek, total):
        ...
        uiwrapper.update_percentagelabel_post("text", "{:.2f}".format(round((seek / total) * 100, 2)) + "%")
    ...
    if uiwrapper.get_srclanguagecodeinput() == "":
        ...
        print(file)
        result = model.transcribe(file, verbose=True, language=uiwrapper.get_srclanguagecodeinput(), progress_callback=progress_notify)
        result.to_srt_vtt(transcribedsrtfile, word_level=False)
        #writer = get_writer("srt", os.path.dirname(file))
        #writer(result, transcribedsrtfile)
...
def transcribe_from_mp3_whisper(file1, file2, uiwrapper):
    if os.path.splitext(file1)[1] != ".mp3":
        ...
        transcribe_stable_whisper(file2, uiwrapper.get_translateoption_Var(), uiwrapper)
    else:
        transcribe_stable_whisper(file2, uiwrapper.get_translateoption_Var(), uiwrapper)
...
def testfunc(transcribedsrcfile):
    with open(transcribedsrcfile, 'r', encoding='utf-8', errors='ignore') as orig_srt:
        orig_srt_text = orig_srt.read().splitlines()
    ...
    if not orig_srt_text:
        ...
        if get_byte_size(src_lang_sentence) > 8192:
            src_lang_sentences = split_string_by_max_byte(src_lang_sentence)
            for sentence in src_lang_sentences:
                trg_lang_sentences.append(translateusingapifortest(sentence))
            trg_lang = ''.join(trg_lang_sentences)
            makesrtfortest(timestamps, trg_lang, transcribedsrcfile)
        else:
            tr = translateusingapifortest(src_lang_sentence)
            makesrtfortest(timestamps, tr, transcribedsrcfile)
